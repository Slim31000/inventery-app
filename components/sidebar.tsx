        ...
        <nav className='space-y-1'>
            ...
            {navigation.map((item,key)=>{
                ...
                <Link href={item.href} key={key} className={`flex items-center space-x-3 py-2 px-3 rounded-lg ${isActive?"bg-purple-100 text-gray-800":"hover:bg-gray-800 text-gray-300"}`} >
                    <IconComponent className='w-5 h-5 mr-2'/> <span className='text-sm'>{item.name}</span>
                    </Link>
            )})}

        </nav>
        <div className='absolute bottom-0 left-0 right-0 pa-6 borter-t border-gray-700'>
